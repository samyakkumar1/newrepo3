...
(function () {
    ...
    window.svgEditor = (function ($) {
        ...
        editor.init = function () {
            ...
            var updateContextPanel = svgEditor.updateContextPanel = function () {
                ...
                $("#popup_toolbar").children().appendTo(".invisibleDiv");
                $("#popup_toolbar").remove();
                var div = $("<div id='popup_toolbar' class='popup_toolbar_style'></div>");

                var addColorToolsToDiv = function (color) {
                    $("#colorTools").appendTo(div);
                    if (color == null) {
                        color = "FFFFFF";
                    }
                ...
                var repositionToolBar = function () {
                    if (lastToolBarPosition == undefined || lastToolBarPosition.left == undefined) {
                        $(div).css("left", ($("body").width() / 2) - ($(div).width() / 2));
                        $(div).css("top", "-45px");
                    } else {
                        $(div).css("left", lastToolBarPosition.left);
                        $(div).css("top", lastToolBarPosition.top);
                    }
                    $(div).draggable({
                        stop: function () {
                            lastToolBarPosition = {left: $(div).css("left"), top: $(div).css("top")};
                        },
                        containment: "parent", scroll: false
                    });
                    //$(div).css("top", $("#toolbar_top").position().top);
                };
                if (elem != null) {
...
//					}
                    ...
                    if (panels[el_name]) {
                        ...
                        if (el_name == 'text') {
                            ...
                            $("#textTools").appendTo(div);

                            var font = $(elem).attr("font-family"); 
                            var fontListElement = $("#preview td").filter(function () {
                                if ($(this).attr("data-value") == font) {
                                    return true;
                                }
                                var alias = $(this).attr("data-alias");
                                var found = false;
                                if (alias != undefined && alias != "undefined") {
                                    alias = alias.split(","); 
                                    alias.forEach(function (val, idx) {
                                        if (font == val) {
                                            found = true;
                                        }
                                    }); 
                            ...
                            $("#fontPreviewList").html(fontListElement.text()  + "<span class='fontArrow'></span>");
                            var font_size = parseInt(parseInt($(elem).attr("font-size")));
                            option = $("#comboFontSize option").filter(function () {
                                return $(this).text() == font_size;
                            });
                            $(option).attr('selected', true);
                            addColorToolsToDiv($(elem).attr("fill"))
                        } // text
                        else if ((elem.id == "logo") || (el_name == 'image') || $(elem).find("svg").length > 0) {
                            setImageURL(svgCanvas.getHref(elem));
                            $("#imgTools").appendTo(div);
                            //$("#insertImageToolBar")
                        } // image
                            ...
                            addColorToolsToDiv(getColor(elem));
                            $("#unGroupTools").appendTo(div);
                        } else {
                            addColorToolsToDiv($(elem).attr("fill"));
                        }
                    } else {
                    ...
                    /*
                     if (el_name == 'path' || el_name == 'polygon'){
                     addColorToolsToDiv($(elem).attr("fill"));
                     }
                     */
                    repositionToolBar();
                    ...
                    div.appendTo('body');
                    addColorToolsToDiv($(svgCanvas.getSelectedElems()[0]).attr("fill"));
                    $("#groupTools").appendTo(div);
                    repositionToolBar();
...
//						|| elem && elem.tagName == "path" && !multiselected) { // This was added in r1430, but not sure why
            ...
            changeZoom = editor.changeZoom = function (ctl) {
...
function addBorderElement() {
    var cbg1 = $("#canvasBackground");
    var svg2obj = $("#canvasBackground2");
    if (svg2obj.length == 0) {
        var svgc2 = '<svg id="canvasBackground2" width="640px" height="480px" x="640px" y="480px" overflow="none" style="pointer-events:none">' +
            '<rect id="addedBorderDesign" x="10px" y="10px" stroke-dasharray="5,5,5" stroke-opacity="0.5" stroke="#FF0000" stroke-width="2px" fill="#FFFFFF" fill-opacity="0" style="pointer-events:none"></rect>' +
            '</svg>';
        svg2obj = $(svgc2);
    }
    svg2obj.attr("width", cbg1.attr("width"));
    svg2obj.attr("height", cbg1.attr("height"));
    svg2obj.attr("x", cbg1.attr("x"));
    svg2obj.attr("y", cbg1.attr("y"));
    cbg1.parent().append(svg2obj);
    cbg1.attr("width", cbg1.attr("width"));
    cbg1.attr("height", cbg1.attr("height"));
    cbg1.attr("x", cbg1.attr("x"));
    cbg1.attr("y", cbg1.attr("y"));
    $("#addedBorderDesign").attr("width", $("#canvasBackground2").attr("width") - 20);
    $("#addedBorderDesign").attr("height", $("#canvasBackground2").attr("height") - 20);
    $("#svgcontent").attr("overflow", "invisible");
}
