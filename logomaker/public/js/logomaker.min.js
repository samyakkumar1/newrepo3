						 ...
						 if (obj.attr("data-url")) {
 							if (states[3] != "edit") {
								...
								chooseCategory(obj.attr("data-id"), '', function () {
									  console.log("urlChanged>else> step2> edit> chooseCategory")
 										...
 										doPost("/getLogoDetails", {id: obj.attr("data-id")}, function (resp) {
 											...
 											if (resp.status == 200) {
 												showPanelThree(function () {
													 ...
													 console.log(states+" : "+ resp.logo.s3_logo_url)
...
function init() {
	...
	$("#industry-sub-key-search").on("keyup", function () {
		var states = history.state.name.split("/");
		var obj = $("[data-url='" + states[0] + "']") || $("[data-id='" + states[0] + "']");
		var keywords = "%" + $("#industry-sub-key-search").val().trim() + "%";

		chooseCategory(obj.attr("data-id"), keywords, function () {
			if (states[2] == "edit") {
				showPanelThree(function () {
					if (savedLogoId.indexOf("logo_") == 0) {
						addWaitingOverlay();
	...
	function chooseCategory(id, keywords, callback) {
		console.log(id)
		if (loadedCategories.indexOf(id) < 0 || keywords !='') {
			addWaitingOverlay();
