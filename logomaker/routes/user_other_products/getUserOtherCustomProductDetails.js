    ...
    if (req.user) {
        ...
        if (req.query.from == "mydesigns"){
            easydb(dbPool)
                .query(function () {
                    return {
                        query: "SELECT other_product_details.title,other_product_details.description,other_products_images.color, other_product_details.front_image, other_product_details.back_image,user_other_products_custom.* FROM user_other_products_custom LEFT JOIN other_product_details ON other_product_details.id = user_other_products_custom.base_product_id LEFT JOIN other_products_images ON other_product_details.other_product_id = other_products_images.id WHERE user_other_products_custom.id = ? AND user_other_products_custom.user_id = ?",
                ...
                .success(function (rows) {
                    if (rows.length > 0){
                        product = rows[0]; 
                        res.send({product: product, settings: undefined, status: 200}); 
                    } else {
