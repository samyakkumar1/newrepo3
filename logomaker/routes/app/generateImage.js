...
exports.index = function (req, res) {
    ...
    if (!req.query.url){
        res.send(400, "url must be specified.");
    } else {
...
exports.post = function (req, res) {
    var options = {
        ...
        id: undefined,
        from: undefined,
        url: "data:image/svg+xml;base64," + new Buffer(decodeURIComponent(req.body.data)).toString('base64'),
        cmyk: false,
        height: '173',
        width: '300',
        callback: function (buff) {
            res.send("data:image/" + options.format + ";base64," + buff.toString('base64'));
        }
    };
    generateImage(options, res, req);
};
...
function generateImage(options, res, req) {
    ...
    if (options.url.indexOf("data:") == 0) {
        ...
        fileName = "file:///" + (options.type == "card" ? (options.from == "base" ? config.CARD_PATH : config.USER_CARD_PATH) : (options.from == "base" ? config.LOGO_PATH : config.USER_LOGO_PATH)) + "/" + options.url;
    } 
                    ...
                    if (status !== 'success') {
                        ...
                        page.render(fileName, {quality: options.quality}).then(function () {
                            if (options.callback) {
                                ...
                                var stream;
                                if (options.format == "pdf" && (options.cmyk || options.type == "card")) {
