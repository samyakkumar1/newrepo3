...
var scheduler = require('./scheduler'),
    ...
    searchSubCategories = require('./routes/category_master/searchSubCategories'),
    ...
    blogs=require('./routes/blogs/blog');
...
var mainFn = function() {
    ...
    app.disable('etag');
    app.use(function(req, res, next) {
        if (req.url.indexOf("/img/") >= 0 || req.url.indexOf("/css/") >= 0 || req.url.indexOf("/js/") >= 0 || req.url.indexOf("/fonts/") >= 0 || req.url.indexOf("/svgedit/") >= 0 || req.url.indexOf("/vendor/") >= 0) {
    ...
    app.get('/logodesigner', function(req, res, next) {
        res.redirect("/logo-design-online");
    });
    app.get('/logo-design-online/checkout', pages.checkout);
    app.get('/logo-design-online', pages.logodesigner);
    app.get('/logo-design-online/*', pages.logodesigner);

    app.get('/design-business-card-online/checkout', pages.checkout);
    app.get('/design-business-card-online/*', pages.bcdesigner);

    app.get('/signup', pages.signup);
    app.get('/bcdesigner', function(req, res, next) {
        res.redirect("/design-business-card-online");
    });
    app.get('/design-business-card-online', pages.bcdesigner);
    ...
    app.get('/blog', function(req, res, next) {
        res.redirect("/blog-online-free-logo");
    });
    app.get('/blog-online-free-logo', pages.blog);
    ...
    app.get('/contact-us', pages.contactUs);
    ...
    app.get('/my-designs', pages.mydesigns); 
    app.get("/polo-tshirt", pages.poloTShirt);
    ...
    app.get("*", function(req, res, next) {
        res.status(404).render('error', {
            ...
            error: {
                stack: "We can't find the requested page deepak.",
                status: 404
            },
    ...
    app.post('/searchSubCategories', searchSubCategories.index);
    ...
    var certificate = fs.readFileSync('sslcert/6b1f18fc2aa9d18d.crt', 'utf8');
...
function readSettingsFile() {
    ...
    try {
        ...
        global.config.emailConfig.auth.pass = decrypt(global.config.emailConfig_gmail.auth.pass); 
        global.transporter = nodemailer.createTransport(smtpPool(global.config.emailConfig_gmail)); 
	console.log("User: ", global.config.emailConfig.auth);
