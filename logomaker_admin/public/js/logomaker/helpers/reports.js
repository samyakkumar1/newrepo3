        ...
        doPost("/purchasedItems", data, function(resp){
            ...
            if (resp.status == 200){
                ...
                if (resp.items.length > 0){
                    ...
                    for(var i = 0; i < resp.items.length; i++){
                        var item = resp.items[i];
                        tbl.append("<tr><td>" + item.cart_id + (item.item_type == "BC" ? "BC" : (item.item_type == "OTHER" ? "OT" : (item.item_type == "CUSTOM" ? "CU" : "LG"))) + item.item_id +"</td><td>" +item.first_name+" "+item.last_name+" / "+ item.email_address+"</td><td>"+item.phone_number+"</td><td>"+item.address1+"<br/>"+item.address2+"<br/>"+item.city+"<br/>"+item.state+"<br/>"+item.country+"<br/>"+item.postal_code+"</td><td>"+item.payement_type+"</td>"+"<td>"+(item.item_type == "BC" || item.item_type=="CUSTOM" ? item.qty : 1)+"</td><td>"+(item.item_type == "BC" ? "Card" : item.item_type == "CUSTOM" ? item.name:"Logo")+"</td>"+"<td>"+item.price+" </td><td> "+item.currency + " " + (items[item.cart_id].length - 1 == 0 ? "" : "("+(items[item.cart_id].length - 1) + " more item)") +"</td><td>"+moment(item.date_purchased).format("YYYY-MM-DD hh:mm A")+"</td>"+"<td>"+(item.order_status == "NOT_PRINTED" ? "Not Printed" : (item.order_status == "PRINTING" ? "Printing" : "Printed"))+"</td>"+"</tr>");
                    }
                } else {
